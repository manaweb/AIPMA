<?php
	...
	if ($_GET['faz']=="dados") {
		...
		# Dados;
		$dados = array('id_subcategoria'=>$id_subcategoria, 'nome'=>$nome, 'preco'=>$preco, 'descricao'=>$descricao);

		# Arquivos
	
		if (!empty($_FILES['foto1']['name'])) {
			$dados['foto1'] = processaArquivo('foto1',$Config,$_FILES,1,'imagem');
			if ($dados['foto1'] == false) { header("Location: ../sys/".$Config2['arquivo'].".php?erro=".urlencode('Erro processando Imagem.'),true); exit; }
		}
		...
		if ($$Config['id']>0) {
			...
			db_apagaArquivo('foto1',$Config,$$Config['id']);
	...
	if ($_GET['faz']=="excluir") {
		...
		if ($id>0) {
			...
			db_apagaArquivo('foto1',$Config,$id,$arquivos);

			# Excluindo do Bando de dados
			db_consulta("DELETE FROM ".$Config['tabela']." WHERE id=$id");
